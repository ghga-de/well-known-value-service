...
```bash
docker pull ghga/well-known-value-service:2.1.1
```

Or you can build the container yourself from the [`./Dockerfile`](./Dockerfile):
```bash
# Execute in the repo's root dir:
docker build -t ghga/well-known-value-service:2.1.1 .
```
...
docker run -p 8080:8080 ghga/well-known-value-service:2.1.1 --help
```
